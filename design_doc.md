...
# how to follow vehicle with camera

- define vector of position compared to center of vehicle when upright
- apply vehicle rotation to that
- add camera pitch and yaw to make it work
- almost works ! (weird rendering axis discrepancy)

# planar roads

- store road sections as quads + equation planes
- to check collision -> signed distance <= 0.0 along an axis line
- need to store some overlap
- have to change down directions in some places in collision detection
- have to do minimum nudge by projecting/rotating directions
- have to add that to the Generator trait :
   - if provides collision :
      - check collision on both voxels and generator
      - have a function for both rays and curves
         - optionally (Option<>) returns a special struct if there has been collision :
            - normal of whatever surface got collision
            - kind of "voxel" expected there
            - minimum nudge to get out of surface
...
- still ongoing
- need to find way to reduce cost of aggregating faces 

- how to reduce cost of getting face data : DONE
   - store face data in taken_dirs-like structure
      - a Vec<[Option<(u32, (u8,u8,u8))> ; 6]>
   - populate it once on pass step=1
   - next passes just have to reference that and update it with their step
- if only one solid type, only do outer area

- idea : do all faces in seperate meshes 
   - allows unloading of faces that can't be seen

# how to optimize terrain generation ?

- pre-generate tiles that the server will assemble in real time
- generated by the server at startup so they're always different

# ideal terrain idea

- multiple biomes :
   - each has different kinds of roads
   - some may be assembled from pre-made stuff, others not to give different feels
